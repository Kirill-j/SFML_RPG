...
const int TileMap::getLayerSize(const int x, const int y, const int layer) const
{
	if (x >= 0 && x < static_cast<int>(this->map.size()))
	{
		...
		{
			if (layer >= 0 && static_cast<int>(this->map[x][y].size()))
...
{
	/* Take three indicies from the mouse position in the grid and add a tile to that positionif the internal tilemap array allows it.*/
	...
	{
		/*OK To add tile. */
		this->map[x][y][z].push_back(new Tile(x, y, gridSizeF, this->tileSheet, texture_rect, collision, type));
